//dwellwell-api/src/routes/ai.ts
 ...
 * AI fallback suggestions when DB lookup is sparse.
 * Returns: Array<{brand, model, type, category, notes?, imageUrl?}>
 */
router.get('/lookup-appliance', async (req, res) => {
  // accept both ?q= and ?query= (client is using ?q=)
      ...
      messages: [
        {
          role: 'user',
          content: `
Return a JSON array of 3 likely appliances for: "${q}".
Each item must be:
