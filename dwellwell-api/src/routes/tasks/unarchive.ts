//dwellwell-api/src/routes/tasks/unarchive.ts
...
export default asyncHandler(async (req: Request, res: Response) => {
  ...
  const updated = await prisma.userTask.update({
    where: { id: t.id },
    data: { archivedAt: null, isTracking: true, dueDate: nextDue },
  });

  await prisma.lifecycleEvent.create({ data: { userId, entity: 'task', entityId: t.id, action: 'unarchived', metadata: { mode } } });
  res.json(updated);
